...
- **[AscendNPU IR样例](./examples/)** 旨在提供快速上手、开发和调试AscendNPU IR功能的最小化实现，其核心目标是使用最精简的代码展示AscendNPU IR核心功能，**而非提供生产级的安全保障**。此样例中的安全功能（如输入校验、边界检查等）相对有限。
- AscendNPU IR不推荐用户直接将样例作为业务代码，也不保证此种做法的安全性。若用户将`examples` 目录中的示例代码应用在自身的真实业务场景中且发生了安全问题，则由用户自行承担。
