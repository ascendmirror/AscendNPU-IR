...
Value hfusion::castTo(OpBuilder &builder, Value src, Type targetElemType,
                      hfusion::RoundMode roundMode, hfusion::TypeFn casting,
                      std::optional<Value> dst, bool enableOverflow) {
  Location loc = src.getLoc();
...
Value hfusion::castTo(OpBuilder &builder, Value src, Type targetElemType,
  ...
  hfusion::RoundMode rounding =
      mlir::utils::selectRoundMode<hfusion::RoundMode>(srcElemType,
                                                       targetElemType);
  return hfusion::castTo(builder, src, targetElemType, rounding, casting,
                         std::nullopt,
                         /*enableOverflow=*/true);
}
