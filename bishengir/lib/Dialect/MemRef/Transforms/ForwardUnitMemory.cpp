...
public:
  ...
  LogicalResult matchAndRewrite(memref::StoreOp storeOp,
    ...
    for (auto &usage : uses) {
      ...
      LDBG(usage->getOwner());
      auto memoryEffect =
          memoryModificationOp.getEffectOnValue<MemoryEffects::Read>(
              memoryAllocated);
      if (!memoryModificationOp || !memoryEffect.has_value())
        return rewriter.notifyMatchFailure(
            memoryModificationOp,
            "other usage of store's memref is not a read");
      readLike = usage;
    }
    if (readLike == nullptr)
      return rewriter.notifyMatchFailure(storeOp, "readLike op is null");
    auto *readOp = readLike->getOwner();
    // Check whether they are on the same block
    ...
    DominanceInfo info(storeOp.getOperation()->getParentOfType<func::FuncOp>());
    // Only will catching one read besides storeOp
    ...
    if (!info.dominates(storeOp, readOp)) {
      return rewriter.notifyMatchFailure(readOp, "store doesn't dominate read");
    }

    // Check if the store op was loaded from somewhere
    // Check if there's a write to that too until here
    ...
    if (auto storeFromLoadOp =
      ...
      if (tmpMemref.hasOneUse()) {
        readLike->assign(storeFromLoadOp.getMemRef());
        return success();
      }
    }
    return rewriter.notifyMatchFailure(
