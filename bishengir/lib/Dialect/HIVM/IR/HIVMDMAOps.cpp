...
#include "bishengit/Dialect/HACC/Utils/Utils.h"
...
static LogicalResult checkCopyOpMemSpace(CopyOp &op) {
  ...
  if (srcMemSpaceAttr && dstMemSpaceAttr) {
...
#if defined(BISHENGIR_ENABLE_A5_UNPUBLISHED_FEATURES) &&                       \
    BISHENGIR_ENABLE_A5_UNPUBLISHED_FEATURES
    ...
    if (!kCopySupported.count(std::make_pair(srcAddrSpace, dstAddrSpace))) {
      ...
      std::string errStr{"Unsupported copy from " + srcStr + " to " + dstStr +
                         "!"};
      return op.emitOpError(errStr);
    }
  }
