...
constexpr static StringRef loadOnlyForCubeFromGM = "LoadOnlyForCube";
...
traceDefOpAndSave(Value v,
  ...
  if (Operation *definingOp = v.getDefiningOp<OpType>()) {
    ...
    return traceDefOpAndSave<OpType>(reshapeOp.getSource(), defChain,
                                     isSingleChain);
    ...
    return traceDefOpAndSave<OpType>(castOp.getViewSource(), defChain,
                                     isSingleChain);
  } else if (auto reinterpretCastOp =
                 v.getDefiningOp<memref::ReinterpretCastOp>()) {
    saveOpInfo(reinterpretCastOp, defChain);
    return traceDefOpAndSave<OpType>(reinterpretCastOp.getViewSource(),
                                     defChain, isSingleChain);
    ...
    if (auto scfForOp = dyn_cast_if_present<scf::ForOp>(
      ...
      if (OpOperand *iterArgOperand = scfForOp.getTiedLoopInit(blockArg)) {
        ...
        return traceDefOpAndSave<OpType>(iterArgOperand->get(), defChain,
                                         isSingleChain);
      }
    }
    ...
    return traceDefOpAndSave<OpType>(extractSliceOp.getSource(), defChain,
                                     isSingleChain);
  } else if (auto insertSliceOp = v.getDefiningOp<tensor::InsertSliceOp>()) {
    saveOpInfo(insertSliceOp, defChain);
    return traceDefOpAndSave<OpType>(insertSliceOp.getSource(), defChain,
                                     isSingleChain);
  }
  return std::nullopt;
}
...
Operation *getLatestFromMapping(Operation *oldOp, IRMapping &mapping) {
    ...
    for (Operation *user : iterOp->getResult(0).getUsers()) {
      if (isMemrefViewLikeOp(user)) {
        ...
        if (user->getBlock() == limitBlock)
          return user;
        return failure();
      } else if (isa<scf::ForOp, scf::WhileOp, scf::IfOp, scf::ParallelOp>(
                     user)) {
        return failure();
      }
    }
...
/// If the load is passed by a initArgs,
/// add an additional regionIterArg.
...
LogicalResult cloneDefChainForCubeOp(
  ...
  while (defChain.size() > 0) {
    ...
    Operation *opToBeCloned = getLatestFromMapping(opInfoPair.first, mapping);
...
cloneLoadForCubeOp(PatternRewriter &rewriter,
  ...
  for (Operation *op : deps) {
    ...
    if (lastInsertOp && isa<hivm::LoadOp>(lastInsertOp)) {
      lastInsertOp->setAttr(loadOnlyForCubeFromGM,
