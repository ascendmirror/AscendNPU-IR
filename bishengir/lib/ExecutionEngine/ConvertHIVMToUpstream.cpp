...
reallocShapedValue(PatternRewriter &rewriter, ShapedValue value,
  ...
  auto dimMaker = [value, dimGetter,
    ...
    for (auto dimIdx : llvm::seq(type.getRank())) {
      ...
      if (dim.is<Attribute>() &&
          cast<IntegerAttr>(dim.get<Attribute>()).getUInt() == 0)
        break;
...
struct RewriteElemwiseOp final : public GenericPreprocessAndRewrite<From> {
  static_assert(
      llvm::is_one_of<To, hfusion::ElemwiseUnaryOp, hfusion::ElemwiseBinaryOp,
                      linalg::ElemwiseUnaryOp, linalg::ElemwiseBinaryOp>::value,
      "Cannot use this \"To\" type!");
  static_assert(
      std::is_same_v<
          decltype(equivalentFn),
          SwitchFinder_t<To, hfusion::ElemwiseUnaryOp, hfusion::UnaryFn,
                         hfusion::ElemwiseBinaryOp, hfusion::BinaryFn,
                         linalg::ElemwiseUnaryOp, linalg::UnaryFn,
                         linalg::ElemwiseBinaryOp, linalg::BinaryFn>>,
      "Cannot use this \"equivalentFn\" parameter!");
  ...
  LogicalResult rewriteFromGeneric(From op,
    ...
    elemwiseOp.setFun(equivalentFn);
    return success();
  }
};
...
struct RewriteVReduceOp : public OpRewritePattern<hivm::VReduceOp> {
  ...
  LogicalResult matchAndRewrite(hivm::VReduceOp op,
    ...
    const auto dstsWithoutDim =
        llvm::map_to_vector(dsts, [&](Value dst) -> Value {
          return reallocShapedValue(rewriter, cast<ShapedValue>(dst), loc,
                                    [&](int64_t dimIdx) -> OpFoldResult {
                                      if (dimIdx == firstReduceDim)
    ...
    case hivm::ReduceOperation::min_with_index_left:
      reduceOp = rewriter.create<hfusion::ReduceWithIndexOp>(
          loc, ValueRange(dstsWithoutDim).getTypes(), src, dstsWithoutDim,
          rewriter.getAttr<hfusion::ReduceWithIndexKindAttr>(
              hfusion::ReduceWithIndexKind::MIN),
          rewriter.getBoolAttr(true), op.getReduceDimsAttr());
      break;
    case hivm::ReduceOperation::min_with_index_right:
      reduceOp = rewriter.create<hfusion::ReduceWithIndexOp>(
          ...
          rewriter.getAttr<hfusion::ReduceWithIndexKindAttr>(
              hfusion::ReduceWithIndexKind::MIN),
          rewriter.getBoolAttr(false), op.getReduceDimsAttr());
      break;
    case hivm::ReduceOperation::max_with_index_left:
      reduceOp = rewriter.create<hfusion::ReduceWithIndexOp>(
          loc, ValueRange(dstsWithoutDim).getTypes(), src, dstsWithoutDim,
          rewriter.getAttr<hfusion::ReduceWithIndexKindAttr>(
              hfusion::ReduceWithIndexKind::MAX),
          rewriter.getBoolAttr(true), op.getReduceDimsAttr());
      break;
    ...
    default:
      ...
      reduceOp = rewriter.create<linalg::ReduceOp>(
          loc, src, dstsWithoutDim, op.getReduceDims(),
    ...
    auto expandedReduceResults = llvm::map_to_vector(
        llvm::zip(op.getResultTypes(), reduceOp->getResults()),
        [&](auto zippedResult) -> Value {
          ...
          const auto reassociation = reshape_utils::getReAssociation(
              {firstReduceDim}, cast<ShapedType>(originalResType).getRank());
    ...
    if (hasPureTensorSemantics)
      ...
      for (auto [src, dst] : llvm::zip(op.getDst(), expandedReduceResults))
        rewriter.create<bufferization::MaterializeInDestinationOp>(loc, src,
                                                                   dst);
      rewriter.eraseOp(op);
    }
