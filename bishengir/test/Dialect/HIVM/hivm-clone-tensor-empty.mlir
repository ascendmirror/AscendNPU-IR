...
// CHECK:   %[[INS:.*]] = tensor.insert %c42_i32 into %[[EMPTY]][%c0] : tensor<1xi32>
// CHECK:   %[[RC:.*]] = memref.reinterpret_cast %arg0 to offset: [%[[IV]]]{{.*}} : memref<?xi32> to memref<1xi32, strided<[1], offset: ?>>
// CHECK:   hivm.hir.store ins(%[[INS]] : tensor<1xi32>) outs(%[[RC]] : memref<1xi32, strided<[1], offset: ?>>)
...
module {
  func.func @rewrite_tensor_before_use(%arg0: memref<?xi32>) {
    ...
    // Original empty outside loop
    %7 = tensor.empty() : tensor<1xi32>

    scf.for %arg1 = %c0 to %c4096 step %c1 {
      %inserted = tensor.insert %c42_i32 into %7[%c0] : tensor<1xi32>
      %reinterpret_cast =
        memref.reinterpret_cast %arg0
        to offset: [%arg1], sizes: [1], strides: [1]
        : memref<?xi32> to memref<1xi32, strided<[1], offset: ?>>
      hivm.hir.store
        ins(%inserted : tensor<1xi32>)
        outs(%reinterpret_cast : memref<1xi32, strided<[1], offset: ?>>)
    }
