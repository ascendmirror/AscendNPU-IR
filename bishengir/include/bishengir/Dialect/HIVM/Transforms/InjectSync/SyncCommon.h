...
struct BaseMemInfo {
  BaseMemInfo(
      ...
      : baseBuffer(baseBuffer), rootBuffer(rootBuffer), scope(scope),
        baseAddresses(baseAddresses), allocateSize(allocateSize),
  ...
  std::unique_ptr<BaseMemInfo> clone(Value cloneBaseBuffer) const {
    std::unique_ptr<BaseMemInfo> newMemInfo = std::make_unique<BaseMemInfo>(
        BaseMemInfo{cloneBaseBuffer, rootBuffer, scope, baseAddresses,
                    allocateSize, allocWorkspaceOp});
    return newMemInfo;
  }
};
...
using Buffer2MemInfoMap = DenseMap<Value, std::unique_ptr<BaseMemInfo>>;
